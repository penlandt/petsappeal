<x-app-layout>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 shadow-sm rounded p-6">
            <form method="POST" action="{{ route('boarding.units.update', $boardingUnit->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input name="name" type="text" required
                           ...
                           value="{{ old('name', $boardingUnit->name) }}" />
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="type" required class="mt-1 block w-full rounded-md shadow-sm"
                            style="background-color: #fff; color: #000;">
                        @foreach (['kennel', 'cage', 'room', 'unit'] as $type)
                            <option value="{{ $type }}" {{ old('type', $boardingUnit->type) == $type ? 'selected' : '' }}>
                                {{ ucfirst($type) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="size" required class="mt-1 block w-full rounded-md shadow-sm"
                        ...
                        @foreach (['small', 'medium', 'large', 'extra-large'] as $size)
                            <option value="{{ $size }}" {{ old('size', $boardingUnit->size) == $size ? 'selected' : '' }}>
                                {{ ucfirst($size) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input name="max_occupants" type="number" min="1" required
                           class="mt-1 block w-full rounded-md shadow-sm"
                           style="background-color: #fff; color: #000;"
                           value="{{ old('max_occupants', $boardingUnit->max_occupants) }}" />
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input name="price_per_night" type="number" step="0.01" required
                           class="mt-1 block w-full rounded-md shadow-sm"
                           style="background-color: #fff; color: #000;"
                           value="{{ old('price_per_night', $boardingUnit->price_per_night) }}" />
                </div>
                    ...
                    <select name="location_id" required class="mt-1 block w-full rounded-md shadow-sm"
                        ...
                        @foreach ($locations as $location)
                            <option value="{{ $location->id }}"
                                {{ old('location_id', $boardingUnit->location_id) == $location->id ? 'selected' : '' }}>
